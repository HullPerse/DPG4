...
async function rotateAutoBackups() {
  try {
    ...
    logger.error(`Auto-backup rotation failed: ${err instanceof Error ? err.message : String(err)}`)
  }
}
...
async function doBackup(): Promise<boolean> {
  try {
    ...
    if (!(await Bun.file(config.dbPath).exists())) {
      logger.warn(`Auto-backup skipped: database file not found ${config.dbPath}`)
      return false
    }
    ...
    logger.error(`Auto-backup failed: ${err instanceof Error ? err.message : String(err)}`)
    return false
  }
}
