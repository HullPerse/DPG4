...
import PetService, { calcDecayed, DECAY_PER_HOUR } from "@/services/pet.service"
...
export default new Elysia({ prefix: "/pets" })
  ...
  .post(
    ...
    async ({ params, db }) => {
      ...
      logger.setAuthor(sleepUser?.username ?? "SYSTEM").info("put pet to sleep")
  ...
  .post(
    ...
    async ({ params, db, economyService, userService }) => {
      ...
      if (isMoney) {
        ...
        logger.setAuthor(rewardUser?.username ?? "SYSTEM").info(`daily reward money ${moneyAmount}`)
      } else {
        ...
        itemId = item.id
        broadcast("pets", "update", params.userId)
        logger.setAuthor(rewardUser?.username ?? "SYSTEM").info(`daily reward item ${item.label}`)
      }
  ...
  .post(
    ...
    async ({ params, db, economyService }) => {
      ...
      logger.setAuthor(resurrectUser?.username ?? "SYSTEM").info("resurrected pet")
  ...
  .post(
    ...
    async ({ params, db, economyService }) => {
      ...
      logger.setAuthor(searchUser?.username ?? "SYSTEM").info("searched dead pet")
  ...
  .post(
    ...
    async ({ params, db, body }) => {
      ...
      logger.setAuthor(colorUser?.username ?? "SYSTEM").info(`changed pet color ${body.color}`)
  ...
  .post(
    ...
    async ({ params, db, body }) => {
      ...
      logger.setAuthor(modelUser?.username ?? "SYSTEM").info(`changed pet model ${body.model}`)
