...
function measureTableSizes(): Record<string, number> {
  ...
  for (const table of TABLES) {
    try {
      ...
      if (!cols.length) { result[table] = 0; continue }
      const sumExpr = cols.map((c) => `COALESCE(LENGTH(CAST("${c.name}" AS BLOB)), 0)`).join(" + ")
      const row = rawDb.prepare(`SELECT SUM(${sumExpr}) AS bytes FROM "${table}"`).get() as { bytes: number | null }
...
const sentinelRoute = new Elysia({ prefix: "/api" }).get(
  "/sentinel/health",
  async () => {
    const dbOk = checkDb()
    ...
    return {
      ...
      db: { ok: dbOk, path: Bun.env.DB_PATH },
