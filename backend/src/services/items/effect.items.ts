...
export class EffectService {
  ...
  private async consume(ctx: EffectCtx, activityText: string, skipCharge = false) {
    ...
    if (!skipCharge) {
      ...
      await this.economyService.chargeInventory(ctx.inventoryId, inv.charge, -1);
    }
  ...
  private async patchUser(userId: string, patch: Partial<typeof schema.users.$inferInsert>) {
    await this.db
      .update(schema.users)
  ...
  private handlers: Record<string, (ctx: EffectCtx) => Promise<string | null>> = {
    ...
    "Erection - NPC": async ({ userId, inventoryId }) => {
      ...
      if (currentItem) {
        await this.economyService.chargeInventory(inventoryId, currentItem.charge, -1);
      }
    ...
    "Крыса Изгой": async ({ userId }) => {
      ...
      await this.economyService.transferInventoryOwner(currentItem.id, finalItem.owner);
    ...
    "Крыса наркоманка": async ({ userId }) => {
      ...
      await this.economyService.transferInventoryOwner(finalItem.id, userId);
      await this.economyService.addInventory(finalItem.owner, ITEM_DB_IDS.poop);
      await this.userService.score(userId, 10);
    ...
    "Крысиный тапок": async ({ userId, inventoryId }) => {
      ...
      const [inv] = await this.db
        ...
        .from(schema.inventory)
        .where(eq(schema.inventory.id, inventoryId));
      if (inv) await this.economyService.chargeInventory(inventoryId, inv.charge, -1);
    ...
    "Зелье Крысогеддона": async ({ userId }) => {
      ...
      await this.userService.changeStatus(finalUser.id, finalStatus, "remove");
    ...
    "Таинственный предмет": async ({ userId }) => {
      const user = await this.getUser(userId);
      await this.userService.changeStatus(userId, "Таинственный предмет", "add");
  ...
  async executeUse(userId: string, inventoryId: string): Promise<UseItemResult> {
    const [inv] = await this.db
      .select()
      .from(schema.inventory)
      .where(eq(schema.inventory.id, inventoryId));
