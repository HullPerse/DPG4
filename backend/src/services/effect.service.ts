...
export default class EffectService {
  ...
  private handlers: Record<string, (ctx: EffectCtx) => Promise<string | null>> =
    {
      ...
      Кредит: async ({ userId }) => {
        ...
        await this.economyService.removeMoney(userId, 6);
      ...
      Вакуум: async ({ userId }) => {
        ...
        if (nearbyIds.length > 0) {
          ...
          for (const other of allUsers) {
            ...
            usedIds.push(inv[Math.floor(Math.random() * inv.length)]!.id);
          }
      ...
      "Налоговый инспектор": async ({ userId }) => {
        ...
        for (const other of allUsers) {
          ...
          await this.economyService.addMoney(userId, finalValue);
          await this.economyService.removeMoney(other.id, finalValue);
        }
      ...
      Ведро: async ({ userId }) => {
        ...
        const closest = allUsers.filter((d) => d.distance === minDistance)[
          Math.floor(
            Math.random() *
              allUsers.filter((d) => d.distance === minDistance).length,
          )
        ]!;
        const itemIds = [
          ITEM_DB_IDS.bucket1,
      ...
      "Минус 8 чубриков": async ({ userId }) => {
        const user = await this.getUser(userId);
        await this.economyService.removeMoney(userId, 1);
      ...
      "Добрая крыса": async ({ userId }) => {
        ...
        if (ids.length > 0) {
          ...
          for (const id of ids) broadcast("inventory", "update", id);
        }
      ...
      "Крыса гой": async ({ userId }) => {
        ...
        await this.economyService.removeMoney(userId, finalMoney);
  ...
  async executeUse(
    ...
    if (ITEM_MODAL_LABELS.has(inv.label)) {
      await this.inventoryLogService.log("use", inventoryId, userId, userId, {
        mode: "modal",
      });
