...
export default class BlackjackService {
  ...
  private async applyGamblingPayout(userId: string, bid: number, payout: number, devMode?: boolean): Promise<{ banned: boolean; balance: number }> {
  ...
  private async finishGame(game: ActiveGame, devMode?: boolean): Promise<BlackjackState> {
    game.phase = "ended";
    ...
    const { banned, balance } = await this.applyGamblingPayout(game.userId, game.bid, payout, devMode);
    ...
    if (!devMode) {
      ...
      if (user) {
        await this.db.insert(schema.history).values({
          id: newId(), userId: game.userId, owner: { id: user.id, username: user.username },
          type: "blackjack", label, image: "", bid: game.bid, payout, net: -game.bid + payout,
          data: { outcome, playerHand: game.playerHand, dealerHand: game.dealerHand, playerValue: handValue(game.playerHand), dealerValue: handValue(game.dealerHand) },
          created: nowIso(),
        });
      }
      this.logger.info(`blackjack ${outcome} net:${-game.bid + payout}`);
    }
    return this.toState(game, balance, { outcome, payout, net: -game.bid + payout, label, tone, banned });
  }

  private toState(game: ActiveGame, balance: number, result: BlackjackResult | null = null): BlackjackState {
    const ended = game.phase === "ended";
    return {
      phase: game.phase, playerHand: [...game.playerHand],
      dealerHand: ended ? [...game.dealerHand] : [game.dealerHand[0]],
      dealerHoleHidden: !ended && game.dealerHand.length > 1,
      playerValue: handValue(game.playerHand), dealerValue: ended ? handValue(game.dealerHand) : null,
      bid: game.bid, balance, result,
    };
  }

  private async maybeResolveAfterDeal(game: ActiveGame): Promise<BlackjackState | null> {
    if (isBlackjack(game.playerHand)) return this.finishGame(game);
    const dealerUp = game.dealerHand[0];
    if (isPeekCard(dealerUp) && isBlackjack(game.dealerHand)) return this.finishGame(game);
    return null;
  }

  async deal(userId: string, bid: number, devMode?: boolean, devOverrides?: BlackjackDevOverrides): Promise<BlackjackState> {
    if (!devMode) {
    ...
    const game: ActiveGame = { userId, bid, deck: createShoe(), playerHand: [], dealerHand: [], phase: "player" };
  ...
  async hit(userId: string, devMode?: boolean, devOverrides?: BlackjackDevOverrides): Promise<BlackjackState> {
    const game = this.games.get(userId);
    if (!game || game.phase !== "player") throw new Error("No active game");
    const card = devOverrides?.devForceHitCard ? devOverrides.devForceHitCard[0] : draw(game.deck);
    game.playerHand.push(card);
    const updated = devMode ? null : await this.userService.getById(userId);
    const balance = updated?.tickets ?? 0;
    if (handValue(game.playerHand) > 21) return this.finishGame(game, devMode);
    if (handValue(game.playerHand) === 21) { dealerPlay(game.dealerHand, game.deck); return this.finishGame(game, devMode); }
    return this.toState(game, balance);
  }
  ...
  abandon(userId: string): boolean { return this.games.delete(userId); }
}