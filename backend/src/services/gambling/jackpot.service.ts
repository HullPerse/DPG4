...
function generateWinningNumber(): number {
  return Math.floor(Math.random() * JACKPOT_RANGE) + 1;
}
...
export class JackpotService {
  ...
  private async ensureFreshNumber(
    ...
    await this.db
      ...
      .set({ winningNumber: newNumber, winningNumberDate: today, updated: ts })
      .where(eq(schema.jackpot.id, jackpotRow.id));
  }
  ...
  async getStatus() {
    ...
    if (!row) {
      return {
        ...
        lastWinnerUsername: null as string | null,
    ...
    return {
      pool: row.pool,
      lastWinnerId: row.lastWinnerId,
      lastWinnerUsername: row.lastWinnerUsername,
      lastWinAmount: row.lastWinAmount,
      lastWinDate: row.lastWinDate,
    };
  }
  ...
  async addToPool(ticketAmount: number) {
    ...
    if (row) {
      await this.db
        .update(schema.jackpot)
        .set({ pool: row.pool + contribution, updated: ts })
        .where(eq(schema.jackpot.id, row.id));
    } else {
      const id = newId();
      await this.db.insert(schema.jackpot).values({
        id,
        pool: contribution,
