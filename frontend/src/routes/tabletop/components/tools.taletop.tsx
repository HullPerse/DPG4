...
export default function ToolsTaletop({
  ...
  const { noAction, setNoAction, arrowType, setArrowType } = useDataStore(
    useShallow((state) => ({
      noAction: state.noAction,
      setNoAction: state.setNoAction,
      arrowType: state.arrowType,
      setArrowType: state.setArrowType,
    })),
  );
    ...
    <main className="absolute bottom-2 left-2 z-100 flex w-2xl flex-col items-center justify-center gap-2 rounded border-2 border-highlight-high bg-card p-2">
      ...
      <section className="flex h-full w-full flex-col gap-2 border-highlight-high pb-2">
        <div className="flex flex-col">
          ...
          <Select
            value={
              cellsConfig.arrowType.find((item) => item.name === arrowType)
                ?.label
            }
            onValueChange={(e) =>
              setArrowType(
