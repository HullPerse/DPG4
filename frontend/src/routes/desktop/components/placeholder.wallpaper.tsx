...
export function WallpaperUploadPlaceholder({
  ...
  return (
    <div
      ...
      title={fileSizeBytes ? `${fileName} · ${sizeMb} МБ` : fileName}
      aria-busy="true"
    ...
    >
      ...
      <div className="bg-highlight-low relative h-26 w-full">
        <Image src={previewUrl} alt={fileName} className="h-26 w-full object-contain opacity-50" />
        <div className="bg-background/75 absolute inset-0 flex flex-col items-center justify-center gap-1">
          <span className="text-primary text-2xl font-bold tabular-nums">{clamped}%</span>
          <span className="text-muted px-2 text-center text-xs">{stage}</span>
        </div>
      </div>

      <section className="border-t-2 p-1">
        <div className="line-clamp-1 truncate text-center font-bold">{fileName}</div>
