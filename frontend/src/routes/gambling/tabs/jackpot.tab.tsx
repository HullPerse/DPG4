        ...
        useUserStore.setState((s) => {
          ...
          return {
            user: {
              ...
              gamblingBanned: "banned" in res ? res.banned : u.gamblingBanned,
            },
          };
        });
      }
    ...
    <main className="flex flex-col w-full h-full gap-3 p-2 overflow-y-auto">
      <section className="flex flex-col gap-2 w-full border-2 border-highlight-high bg-background p-3">
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-2 text-muted">Текущий джекпот</span>
      ...
      <section className="flex flex-col gap-2 w-full border-2 border-highlight-high bg-background p-3">
        <h3 className="flex items-center gap-2 font-bold text-lg">Испытать удачу</h3>
        <p className="text-sm text-muted">
          Стоимость попытки: <span className="font-bold text-base">10 тикетов</span>
        </p>
        ...
        <section className="flex flex-col gap-2 w-full border-2 border-highlight-high bg-background p-3">
          {lastResult.win ? (
            <div className="flex flex-col items-center gap-2 py-4">
              ...
              <span className="text-sm text-muted">Число: {lastResult.chosen}</span>
              {lastResult.banned && (
