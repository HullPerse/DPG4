  ...
  const handleRoll = useCallback(
    async (type: "general" | "refresh") => {
      ...
      if (!free && type === "general") {
        ...
        if (currentScore < calculateCostSync()) return;

        await userApi.scoreUser(String(user?.id), -calculateCostSync());
      }
      ...
      <div className="flex flex-row gap-1 w-full items-center justify-center">
        <Button
          ...
          disabled={
            rolling.isRolling ||
            list.length === 0 ||
            (!free && Number(user?.money) < calculateCostSync())
          }
        ...
        >
          {rolling.isRolling
            ? "ВРАЩЕНИЕ..."
            : !free
              ? `КРУТИТЬ ЗА ${calculateCostSync()} чб. [${user?.money} всего]`
              : "КРУТИТЬ"}
        </Button>
        {!free && (
          <Button
