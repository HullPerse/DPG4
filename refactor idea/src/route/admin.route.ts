...
import { AdminJwtPayload, verifyAdmin } from "@/lib/admin/admin.utils";
import { broadcastAdminReload, broadcast } from "@/lib/websocket.utils";
...
import { getItem, getUser } from "@/lib/index.utils";
...
const adminRoute = new Elysia()
  .use(databasePlugin)
  .use(servicePlugin)
  .use(jwt({ name: "adminJwt", secret: Bun.env.JWT_SECRET ?? "", exp: "24h" }))
  .group("/api/admin", (app) =>
    app
      .post(
        "/auth",
        async ({ body, db, adminJwt }) => {
          ...
          logger.setAuthor(user.username).info(`logged to admin panel`);
      ...
      .get("/stats", async ({ headers, adminJwt, db, set }) => {
        ...
        if (verified) return { counts: await getAdminStats(db) };
      ...
      .post(
        ...
        async ({ body, db, headers, adminJwt, set, economyService }) => {
          ...
          const user = await getUser(db, body.userId);
          const item = await getItem(db, body.itemId);

          logger
            .setAuthor(user.username)
            .info(`admin granted item ${body.itemId}`);
          return { ok: true };
        },
